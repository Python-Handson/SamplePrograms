...
WRITE_FILE_PATH = READ_FILE_DIR + r'\集計結果.xlsx'
...
def main():
    ...
    df_excel_data = pd.read_excel(READ_FILE_PATH)
    ...
    #------------------------------------------------
    # グラフ描画
    fig,ax = draw_barplot(_x=df_output_data.index, _y=df_output_data['スマートフォン'], _size=(12,8), _title='都道府県別スマホ普及率', _xrot=90)
    ...
    #------------------------------------------------
    # 列のフォーマット
    df_output_data['スマートフォン'] = df_output_data['スマートフォン']/100 # %表示用に100で割る

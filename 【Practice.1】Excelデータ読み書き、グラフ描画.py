...
def main():
    ...
    df_excel_data = pd.read_excel(READ_FILE_PATH, skiprows=range(0,5))
    ...
    #  [O] df_output_data
    #------------------------------------------------
    global df_output_data
    # DataFrameの実体をコピー
    df_output_data = df_excel_data.copy()
    df_output_data = df_output_data.iloc[:47] # 47都道府県の行のみ抽出
    df_output_data.index = df_output_data[df_output_data.columns[0]] # indexを都道府県名に変更
    df_output_data.drop(df_output_data.columns[:3], axis=1, inplace=True) # 不要な列を削除
    ...
    #------------------------------------------------
    # グラフ描画
    fig,ax = draw_scatter(_x=df_output_data['パソコン'], _y=df_output_data['スマートフォン'], _label=df_output_data.index)
    # Excelファイル貼り付け用にグラフを画像として保存
    graph_file_path = WRITE_FILE_DIR + '\\' + 'Excel貼り付け用(テンポラリ).png'
    ...
    list_col_name = list(df_output_data.columns)
    for col_name in list_col_name:
        df_output_data[col_name] = df_output_data[col_name]/100 # %表示用に100で割る
        dict_header_format[col_name] = {'width':8, 'rotation':90, 'fillcol':'215967', 'txtcol':'FFFFFF', 'dtxtformat':'0.0%'}
    # Excelファイルへ出力
    ...
    wb = export_excel(_path=WRITE_FILE_PATH, _df=df_corr, _wb=wb, _letter_fmt='0.00_ ', _frz=False, _auto_flt=False, _col_width_=[20,10], _header_height=40, _sheet_name='相関行列', _index_name='')
